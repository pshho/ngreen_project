...
<body>
	...
	<div id="container">
		<section id="memberView">
			<h2>회원 정보</h2>
			<ul>
				<li>아이디: <input type="text" value="${ member.memberId }"
					readonly></li>
				<li>비밀번호: <input type="text" value="${ member.passwd }"
					readonly></li>
				<li>이름: <input type="text" value="${ member.name }" readonly></li>
				<li>성별: <c:if test="${ member.gender eq '남' }">
				...
				<li>생성날짜: <input type="text"
					value="<fmt:formatDate value='${ member.joinDate }' type='both'/>"
					readonly></li>
				<c:if test="${ sessionId == member.memberId }">
					<li id="btn"><a
						href="/memberUpdate.do?memberId=${ member.memberId }"> <input
							type="button" value="수정">
					</a> <a href="/memberDelete.do?memberId=${ member.memberId }&sessionId=${ sessionId }"> <input
