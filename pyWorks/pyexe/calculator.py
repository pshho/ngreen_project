...
def click(key):
    if key == '=':
        try:
            # eval(문자열 계산) → () 안에 문자 수식을 계산하고 숫자로 변환
            value = eval(display.get())   # 입력된 계산값
            result = str(value)[0:10]   # 소수점 포함해서 10자리 - 소수점 뒤에 8자리 숫자까지만 보임
            display.insert(END, '=' + result)
        except:
            display.insert(END, "→ 오류") # 예외 처리
    elif key == 'C':    # 취소버튼을 누르면 입력값 초기화(삭제)
...
for btn_text in num_pad_list:
    ...
    c += 1
    if c > 2:   # column이 2(3열)보다 크면 0(1열)로 설정
        c = 0
        r += 1
