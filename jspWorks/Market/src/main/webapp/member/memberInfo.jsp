...
<body>
	...
	<div class="container my-3">
		...
		<div class="row">
			...
			<div class="form-group row">
				...
				<div class="col-sm-3 my-2">
					...
					<label><input type="radio" name="gender" value="male" checked disabled> 남</label>
					<label><input type="radio" name="gender" value="female" disabled> 여</label>
					</c:if>
					<c:if test="${member.gender eq '여'}">
					<label><input type="radio" name="gender" value="male" disabled> 남</label>
					<label><input type="radio" name="gender" value="female" checked disabled> 여</label>
					</c:if>
				</div>
			</div>
			<div class="form-group row">
		...
		<div class="col-sm-3 my-2">
			<a href="/deleteMember.do?mid=${ member.mid }&sessionId=${ sessionId }" 
			class="btn_check btn btn-warning" onclick="return confirm('정말 삭제하시겠습니까?');">회원탙퇴</a>
		</div>
	</div>
